...
def mock_response():
    ...
    response.get.return_value.json.return_value = {
        "data": {"amount": "106574.53", "base": "BTC", "currency": "USD"}
    }
    return response
...
def test_fetch_prices_data(
    mock_save_to_json_file, mock_sleep, mock_response, monkeypatch
):
    with patch("fetch_prices.requests.Session", return_value=mock_response):
        ...
        assert (
            mock_save_to_json_file.call_count == 2
        )  # to make sure that this function gets called twice
